...
action(state(robot(r1, Hands), Rooms), move(r2), NewState) :-
    member('steel_key', Hands),
    NewState=state(robot(r2,Hands), Rooms).

action(state(robot(r2, Hands), Rooms), move(r1), NewState) :-
    member('steel_key', Hands),
    NewState=state(robot(r1,Hands), Rooms).

action(state(robot(r1, Hands), Rooms), move(r3), NewState) :-
    member('brass_key', Hands),
    NewState=state(robot(r3,Hands), Rooms).

action(state(robot(r3, Hands), Rooms), move(r1), NewState) :-
    member('brass_key', Hands),
    NewState=state(robot(r1,Hands), Rooms).
